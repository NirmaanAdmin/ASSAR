...
// function get_month_table_headers()
// {
//     $module_name = 'per_client';
//     $month_filter = get_module_filter($module_name, 'months');

//     // Get selected months from filter (comma-separated string)
//     if (!empty($month_filter) && !empty($month_filter->filter_value)) {
//         $selected_months = explode(',', $month_filter->filter_value);
//     } else {
//         // Default: current month + previous 4 months
//         $current = date('Y-m');
//         $selected_months = [];
...
//             $selected_months[] = $month_date->format('Y-m');
//         }
//     }

//     // Limit to 5 months max
...
//     // Generate headers for selected months
//     foreach ($selected_months as $month) {
//         $month_date = new DateTime($month . '-01');
//         $month_display = $month_date->format('F Y');
//         $html .= '<th>' . _l($month_display) . '</th>';
//     }

//     $html .= '
//         <th>' . _l('Earned To Date') . '</th>
//         <th>' . _l('Percent Profits') . '</th>
//     </tr>';

//     return $html;
// }

// function get_month_table_footers()
// {
//     $module_name = 'per_client';
//     $month_filter = get_module_filter($module_name, 'months');
...
//     // Generate footer cells for selected months
//     foreach ($selected_months as $month) {
//         $month_date = new DateTime($month . '-01');
//         $month_class = strtolower($month_date->format('M'));
//         $html .= '<td class="' . $month_class . '"></td>';
//     }
...
function get_month_table_headers($months = [])
{
    // Use POST months first
    if (is_array($months) && count($months) > 0) {
        $selected_months = $months;
    } else {
        // Default: current + previous 4
        $selected_months = [];
        ...
        for ($i = 4; $i >= 0; $i--) {
            ...
            $d->modify("-$i months");
            $selected_months[] = $d->format('Y-m');
        }
    }

    $selected_months = array_slice($selected_months, 0, 5);
    usort($selected_months, fn($a, $b) => strtotime($a) - strtotime($b));
    ...
    foreach ($selected_months as $month) {
        $html .= '<th>' . date('F Y', strtotime($month . '-01')) . '</th>';
    }

    $html .= '
        <th>Earned To Date</th>
        <th>Percent Profits</th>
    </tr>';

    return $html;
}
function get_month_table_footers($months = [])
{
    if (is_array($months) && count($months) > 0) {
        $selected_months = $months;
    } else {
    ...
    foreach ($selected_months as $month) {
        $html .= '<td class="' . strtolower(date('M', strtotime($month))) . '"></td>';
    }
